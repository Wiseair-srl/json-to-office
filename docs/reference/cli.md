...
| `PLUGIN_AUTOLOAD`             | `jto dev`                 | on except in production               | `true` lets the server load the plugins it discovers on disk, once at startup. Off, they are listed but cannot be switched on |
