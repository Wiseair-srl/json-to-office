...
| ------------------- | ----------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `theme`             | `string`          | `'consulting'` | Theme name — built-ins are `consulting`, `minimal`, `devportal`, `vermilion`; custom themes are matched case-insensitively from the `customThemes` generation option. An unknown name falls back to `consulting`. See [Themes & styling](/guide/themes).                                                                                                                 |
...
| `qualityProfile`    | `string`          | —              | The [quality profile](/guide/design-quality#profiles) validation judges this document by when the caller names none; a [blueprint](/reference/blueprints) scaffold writes its archetype's profile here. Built-ins: `client-report`, `technical-report`, `executive-report`, `legal-appendix` and `general`, the format default. An unknown name falls back to `general`. |
...
When `page` is omitted, the section inherits the theme's `page` block (the default `consulting` theme uses A4 with 1440-twip margins on every side). See [the theme schema](/reference/theme-schema).
