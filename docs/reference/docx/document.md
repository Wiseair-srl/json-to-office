...
| `metadata`          | `object`          | —           | Document properties (see below).                                                                                                                                                                                                                                                                              |
...
| `pageBreak` | `boolean`                           | `true` (schema) / `false` (built-in themes) | Start the section on a new page. The schema default is `true`, but every built-in theme (`minimal`, `corporate`, `modern`) overrides it to `false` via `componentDefaults.section` — so in practice sections flow continuously; set `pageBreak: true` explicitly to start on a new page. |
