...
| `font`                       | `{ family?, size?, bold?, fontWeight?, italic?, underline?, lineSpacing? }` | `fontWeight` (100–900) overrides `bold`. `lineSpacing` (`{ type: 'single'\|'atLeast'\|'exactly'\|'double'\|'multiple', value? }`) sets the cell paragraph's line spacing — `exactly` gives dense, fixed-height rows |
