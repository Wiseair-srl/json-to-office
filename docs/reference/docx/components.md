...
`heading` and `paragraph` text supports inline markdown (`**bold**`, `*italic*`, `***bold italic***`, `_underscore variants_`), `\n` line breaks, `[text](url)` hyperlinks (internal bookmark links when the URL starts with `#`), `[@id]` cross-references (below), and the placeholders `{PAGE}`, `{TOTAL_PAGES}`, `{DATE}`, `{DATETIME}`, `{YEAR}` and `{SEQ:name}`. Unknown placeholders are kept as literal text. `{SEQ:figure}` is a Word `SEQ` field: every occurrence of the same name counts up in document order — `Figure {SEQ:figure}.` in three captions reads Figure 1, 2, 3 — and the number is written into the field, so headless LibreOffice and the preview PDF show it without recomputing; Word recomputes it on open, so it follows a figure that moves. All text is Unicode NFC-normalized. Image and visual captions support the bold/italic subset.
...
reports; this component is the way to draw a line without it.
...
| `hideBorders`             | `boolean` \| `{ top?, bottom?, left?, right?, insideHorizontal?, insideVertical? }` | no       | —       | Hide all borders or specific sides. Yields to a side named on a cell or column     |
...
| `font`                       | `{ family?, size?, bold?, fontWeight?, italic?, underline?, lineSpacing? }` | `fontWeight` (100–900) overrides `bold`. `lineSpacing` (`{ type: 'single'\|'atLeast'\|'exactly'\|'double'\|'multiple', value? }`) sets the cell paragraph's line spacing — `exactly` pins the line height for dense rows (content, padding and row height rules can still grow the row) |
| `borderColor` / `borderSize` | `string` \| per-side object / `number` (points) \| per-side object          | Per-cell border override. A side named in a per-side object owns its edge — see the border rules below                                                                                                                                                                                  |
...
**Border rules.** Borders cascade per side — cell over column over table — and two rules keep a stated side authoritative:

- A side **named** in a per-side `borderColor`/`borderSize` object on a cell or its column (`borderColor: { "right": "EF4130" }`) always renders as stated: it survives `hideBorders`, which only silences inherited table-level borders. A scalar `borderColor`/`borderSize` on a cell restyles its visible borders without unhiding any.
- A named side **owns its shared edge**. Adjacent cells each carry half of every interior edge, and when the halves disagree Word and LibreOffice pick winners differently — a stated red divider could render in Word and vanish from a PDF preview of the same file. The pipeline therefore resolves every interior edge itself (a named side beats an inherited one; two named sides fall to OOXML's weight rules, where the wider then darker border wins) and writes the winner on both cells, so every viewer draws the same table. Naming one side of an edge is enough — the neighbouring cell does not need the matching statement.
...
| Prop               | Type                          | Required | Default                 | Description                                                                                                                                                                                                                                                                                                                                  |
| ------------------ | ----------------------------- | -------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `options`          | Highcharts config object      | **yes**  | —                       | Passed to the export server as written, except that a missing top-level `colors` is filled from the [theme palette](/guide/charts#theme-palette) and the [theme typography](/guide/charts#theme-typography-in-highcharts-output) is written beneath any text style the config sets; `chart.width` and `chart.height` (numbers) are mandatory |
| `scale`            | `number`                      | no       | —                       | Export scale factor (higher = sharper raster)                                                                                                                                                                                                                                                                                                |
| `resources`        | `{ css?, js?, files? }`       | no       | —                       | Forwarded to the export server; enables custom `@font-face`, plugins. Registered non-safe theme families are inlined as `@font-face` ahead of `css`                                                                                                                                                                                          |
| `serverUrl`        | `string`                      | no       | `http://localhost:7801` | Export server URL override; takes precedence over the `services.highcharts` config, which in turn overrides the default `http://localhost:7801`. A URL outside this machine and its private networks needs `services.highcharts.allowRemote` — see [What leaves the process](/guide/charts#what-leaves-the-process)                          |
