...
| `qualityProfile`    | string                 | —                                    | The [quality profile](/guide/design-quality#profiles) validation judges this deck by when the caller names none. Built-ins: `consulting-deck`, `executive-presentation`, `technical-presentation`. An unknown name falls back to the format default. |
