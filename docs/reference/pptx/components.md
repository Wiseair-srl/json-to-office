...
This page is the full props reference for the four non-chart content components you can place on a slide: `text`, `image`, `shape`, and `table` — plus the two structural ones, `group` and `block`. For the two chart components (`chart` and `highcharts`) see [PPTX charts](/reference/pptx/charts); for slide structure, groups and the grid system see [Slides & grid](/reference/pptx/slides-and-grid).
...
| Prop         | Type                   | Default | Description                                                                                                                                                                                       |
| ------------ | ---------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `x`, `y`     | number (in) \| `"NN%"` | —       | Frame origin, relative to the enclosing frame or slide. Percentages are fractions of the enclosing width and height.                                                                              |
| `w`, `h`     | number (in) \| `"NN%"` | —       | Frame size. Omitted, the frame fills the enclosing extent.                                                                                                                                        |
| `grid`       | GridPosition           | —       | Frame placement on the enclosing grid, as for any component.                                                                                                                                      |
| `gridConfig` | GridConfig             | —       | Grid for grid placements of this group’s descendants, merged over the enclosing grid field by field. Inside a frame the grid spans the frame; the enclosing margin is dropped unless stated here. |
...
| `gap`        | number (in)            | `0`     | Space between distributed cells.                                                                                                                                                                  |
| `weights`    | number[]               | —       | Relative cell sizes in child order, e.g. `[1.1, 1]` for a 1.1 : 1 split. Missing entries count as 1.                                                                                              |
...
Invoke a JSON block defined in the presentation's `props.blocks`. The two props are the whole contract: `ref` names a definition in **this** document, and `slots` supplies its content. No coordinates are accepted here, and a component placed in a slot may not carry `x`, `y`, `w`, `h` or `grid`: geometry lives in the definition. See [JSON blocks](/reference/blocks#pptx).

| Prop    | Type                      | Default | Description                                                                                   |
| ------- | ------------------------- | ------- | --------------------------------------------------------------------------------------------- |
| `ref`   | string                    | —       | **required.** Name of a definition in `props.blocks`. An unknown name fails validation.       |
| `slots` | `Record<string, unknown>` | —       | Values keyed by the slot names the definition declares; unknown slots and budgets are errors. |
...
- [Slides & grid](/reference/pptx/slides-and-grid) — slide props, groups and frames, grid resolution
- [JSON blocks](/reference/blocks#pptx) — document-local definitions, slots, slide effects, fit
