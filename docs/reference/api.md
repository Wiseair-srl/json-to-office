...
All functions accept an optional [`JsonGenerationOptions`](#jsongenerationoptions) as their last parameter. `Document` is the [docx](https://github.com/dolanmiu/docx) library's document object. Hand it to `saveDocument` to retain deterministic package normalization.
...
| `saveDocument`             | `(document: Document, filename, options?) => Promise<void>`     | Write a generated `Document` to disk with deterministic package normalization and floating-image ID deduplication.                                                                                                                                    |
...
| `deterministic`                 | `boolean`                             | `true`  | Normalize volatile metadata, relationship IDs, and ZIP timestamps for byte-identical output.                                                                                             |
| `generatedAt`                   | `string \| Date`                      | fixed   | Stable document/package timestamp; must be valid and no earlier than 1980.                                                                                                               |
...
| Function                     | Signature                                                     | Description                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `generatePresentation`       | `(document, options?, warnings?) => Promise<PptxGenJS>`       | Low-level entry: returns the pptxgenjs instance. Validates the document first (unless `validation.enabled === false`), throwing `PresentationValidationError` on schema errors; also throws if the root is not `pptx` or an image declares multiple sources. Pass a `PipelineWarning[]` to collect warnings. |
...
| `generateFromFile`           | `(filePath, outputPath, options?) => Promise<void>`           | `.pptx.json` file in, `.pptx` file out.                                                                                                                                                                                                                                                                      |
| `savePresentation`           | `(pptx: PptxGenJS, outputPath, options?) => Promise<void>`    | Write a `generatePresentation` result with deterministic package normalization.                                                                                                                                                                                                                              |
...
::: warning Validation at generation
Like DOCX, PPTX generation runs the deep schema validator by default and throws `PresentationValidationError` on schema errors (opt out with `options.validation.enabled = false`). Pipeline warnings cover render-time issues that pass the schema. To get the full error list without generating, run `validate.document(...)` from the same package (or `jto pptx validate`). See [Validation](/guide/validation).
:::
...
| Option                          | Type                                  | Default | Description                                                                                                                                                                  |
| ------------------------------- | ------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `validation.enabled`            | `boolean`                             | `true`  | Validate the document before rendering; schema errors throw `PresentationValidationError`.                                                                                   |
| `validation.allowUnknownFields` | `boolean`                             | `false` | Strip unknown props instead of rejecting them.                                                                                                                               |
...
| `deterministic`                 | `boolean`                             | `true`  | Normalize volatile metadata, generated IDs, embedded workbooks, and ZIP timestamps for byte-identical output.                                                                |
| `generatedAt`                   | `string \| Date`                      | fixed   | Stable document/package timestamp; must be valid and no earlier than 1980.                                                                                                   |
