...
A slot may also declare a `role` — `actionTitle`, `takeaway`, `source`, `tracker`, `footer` or `logo`. A role says what the content is for so a quality profile can require it (a source under every chart) or measure it (an action title's line count); the theme only styles it, and no role adds a requirement on its own. `tracker`, `footer`, `source` and `logo` also mark chrome, which a deck may draw in the margin band outside the theme's safe area. The `consulting-deck` PPTX profile requires `takeaway` and `source`, bounds the `actionTitle` at two lines, and asks every block-built slide after the cover for a page number; the default profiles ask for nothing. Profile findings are warnings that advise by default and block only under a quality policy gate; a slot the definition itself marks `required` is a validation error regardless of profile.
...
Every definition in `client-report-blocks.docx.json` binds its sizes and colours to the theme's type roles and chrome recipes with a safe default, so the same JSON renders on every bundled theme. The previews below are the house theme, rendered through LibreOffice by `pnpm generate:block-previews` from the invocation each template authors — the example `jto://blocks` publishes — and `pnpm validate:assets` fails when a definition, an example or the theme changes under a preview without it being regenerated. A report built from a cover, three section openers and one running head is generated on all four bundled themes in the test suite, warning-clean, and rendered through LibreOffice where one is installed: every page after the cover carries the document title and a live `n / N`, the sections flow onto one another, and the cover carries neither.

| Block            | Slots                                                                                                                                     | What the definition decides                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cover`          | title ≤ 12 words, subtitle ≤ 20, client ≤ 6, date ≤ 3, confidentiality ≤ 3, logo                                                          | A masthead: the theme's motif at the top edge (a stub of the measure, absent on a theme that declares none), the client in the eyebrow role beneath it, then the cover recipe's rule and the title a third of the way down the body, subtitle under it, and a `Prepared for / Date / Classification` band pinned to the foot of the text area under a hairline. A label whose value is missing is omitted with it. The band's position does not move with the title, so a one-line and a three-line title leave the page equally balanced |
| `key-takeaways`  | label ≤ 8 words on one line (default "Key takeaways"), items 3–5, each one line of ≤ 25 words                                             | The key-takeaways recipe's rule above and below at the recipe's weight (2pt on consulting, a hairline on minimal), the label in the label role, the items as a bulleted list in the body role                                                                                                                                                                                                                                                                                                                                             |
...
| `kpi-row`        | items 2–4 of value ≤ 7 and unit ≤ 5 characters, label, delta, trend; source                                                               | Equal columns of `statistic` in the theme's `stat` role (a scale step smaller when there are four), the delta and its glyph beside the figure and the label under it at the `label` size, a sourced hairline that collapses with the source                                                                                                                                                                                                                                                                                               |
...
Invoked with the definition copied into `props.blocks`, `key-takeaways` takes its label and three to five one-line items; the preview above is this invocation:
...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cover`        | title ≤ 16 words and 110 characters, subtitle ≤ 20 words and 150 characters, client ≤ 8, date ≤ 6, confidentiality ≤ 6, logo (component) | An accent rule from the theme's `cover` recipe, `client · date` in the eyebrow role (the line needs the client), the title in the `title` role stepping down 32 → 28 → 24 → 22 → 20pt over three lines, the subtitle beneath, the logo (slot role `logo`) top right at 18% of the width in the chrome band, confidentiality at the foot |
| `action-chart` | title (action title) ≤ 16 words and 110 characters, tracker, chart (component), takeaway ≤ 28 words and 240 characters, source ≤ 24      | The action title with `fit: { maxLines: 2, shrink: [24, 22] }`, a chart at 68% of the width painted from the theme series, the takeaway beside it under an accent rule, the source beneath a hairline that collapses with it                                                                                                            |
| `kpi-row`      | title, tracker, items 2–4 of value ≤ 7 and unit ≤ 5 characters, label ≤ 6 words and 48 characters, delta ≤ 12 characters; source         | The action title over a row that distributes the items into equal cells (two take half each, four a quarter): value and unit in the `stat` role, the unit in the muted colour, the delta in the label role beneath, the label under that; the source at the foot                                                                        |
...
| `statement`    | assertion (action title) ≤ 11 words, support ≤ 24 words and 150 characters, tracker                                                      | A short accent rule, the assertion in the `display` role stepping down 28 → 24 → 22pt over three lines, the support in the `quote` role beneath: a divider or a closing slide                                                                                                                                                           |

Deck slots bound characters as well as words, because that is what the renderer
wraps by: sixteen ordinary words fit an action title, sixteen compound nouns do
not. The character budgets are what the box holds on the narrowest canvas in the
widest face the theme may fall back to, measured against rendered output (#343);
the word budgets still say what the slot is for.

![cover on consulting, deck](/blocks/cover-deck-consulting.png)

![action-chart on consulting](/blocks/action-chart-deck-consulting.png)

![kpi-row on consulting, deck](/blocks/kpi-row-deck-consulting.png)

![two-column on consulting](/blocks/two-column-deck-consulting.png)

![statement on consulting](/blocks/statement-deck-consulting.png)
...
Bodies can invoke other document-local blocks or explicitly registered code plugins. Plugins can emit blocks or primitives. Missing registrations produce validation errors; references do not install dependencies. Emitted components pass through standard DOCX validation. Expansion is deterministic for JSON-only blocks, with a 64-level / 50,000-node evaluator limit and a 64-level / 100,000-node combined plugin traversal limit. A definition that invokes itself on every expansion — directly or through another definition — is refused at validation, at the `ref` that closes the cycle, with `block_expansion_limit`; recursion under `$if` or `$each`, which the data can end, stops at these limits with the same code. A body that names a block the document does not define is `block_unknown_reference` at that `ref`, inside the definition. Syntax the blocks replaced fails with its replacement named in the suggestion: a deck's `templates`, and a slide's `template`, `placeholders` and `layout`, point to blocks; a component named like a block — one the document defines, or one to copy from `jto://blocks` — is told to invoke it with `"name": "block"`; and an invocation that names its definition with `name` is told to use `ref`.
...
`jto://blocks` provides definitions, derived slot schemas, budgets, a working example and source-template pointers for agentic authoring, in both formats. It carries no pictures, by decision (#334): an agent that wants to see a block previews its example with `jto_preview`, on the renderer, theme and fonts it will generate with, where a bundled PNG would show the house theme on the machine that made it — and would add a megabyte to the package and a converter and an export server to its build. The pictures on this page are generated from the same examples for people reading the reference. Copy selected definitions and any transitive block dependencies into `props.blocks`. The catalog does not register runtime names. Use `jto_workspace_inspect` with `paths: ["/props/blocks"]` to read actual definitions at a workspace revision; `includeBlocks: true` adds definition-derived slot schemas and invocation fill pointers; no catalog entry is inserted into the document implicitly.
