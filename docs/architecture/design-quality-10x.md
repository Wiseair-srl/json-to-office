...
design-evals harness — as a rule pack under the same profile and policy as
the static rules, with a labelled mapping corpus at precision and recall 1.0
on report-shaped documents — so the row is filled by the next matched run.
...
| 7   | Half of overflows escape the static estimator   | ground-truth harness ceiling ≈ 50% strict detection; rendered pass shipped for reports (#344) with a labelled mapping corpus; full cross-format matrix (#343) outstanding                                                                                  | Defects reach the user unless the agent eyeballs every page                                                                                                   |
