...
from the house blocks paints up to eleven sizes (the kpi-row's figure, unit and
label among them), a technical report eight, a consulting deck nine across the
deck and six on its busiest slide; every checkpoint and verification document
of that archetype came in at or under its profile's number.
...
| `client-report`          | DOCX   | Requires a running head with page numbers on every section after the first, `takeaway` and `source` slots wherever a block declares them, promotes heading skips, rendered empty pages and under-filled pages to warnings, enables the three theme-consistency rules with at most eleven sizes, judges the body measure at 45–125 characters, reports empty and untitled sections, headings a page break can strand and unlabelled figures, and requires at least one chart or table; the `client-report` blueprint names it |
