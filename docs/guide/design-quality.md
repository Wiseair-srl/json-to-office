...
- Text authored with `runs` has facts of its own: each run's size counts for
  ...
  safe area. Minimum-size, fit, contrast and density checks still skip it
  rather than guess at mixed run styles.
...
| `docx/heading-hierarchy` | `W_QUALITY_HEADING_SKIP`                                                                                                                                                    | info                                                       | deterministic | A heading jumps down by more than one level; the fix sets the level, except on a heading a block compiled, whose level is the definition's              |
...
| `docx/required-chrome`   | `W_QUALITY_CHROME_MISSING`                                                                                                                                                  | warning; off unless a profile names `required` roles       | deterministic | A block slot with a role the profile requires — a takeaway, a source — left empty or holding only whitespace                                            |
...
| `docx/size-count`        | `W_QUALITY_TYPE_SIZE_COUNT`                                                                                                                                                 | warning; off unless a profile enables it                   | deterministic | More distinct text sizes in the document than `maximumSizes` (8), blocks, statistics, lists and contents entries included; counted per document; no fix |
...
| `docx/body-measure`      | `W_QUALITY_BODY_MEASURE`                                                                                                                                                    | warning; off unless a profile enables it                   | estimated     | A section whose body copy runs outside 45–90 characters a line; the report profiles judge at 125                                                        |
| `docx/section-content`   | `W_QUALITY_SECTION_EMPTY`, `W_QUALITY_SECTION_UNTITLED`                                                                                                                     | warning; off unless a profile enables it                   | deterministic | A section that renders nothing of its own — no heading, contents field, copy or figure — or one carrying 60+ words under no heading                     |
...
| `docx/figure-label`      | `W_QUALITY_FIGURE_UNLABELLED`                                                                                                                                               | warning; off unless a profile enables it                   | deterministic | An image with no alt text and no caption: its own, a figure block's, or a `Figure …` paragraph right before or after it                                 |
| `docx/image-aspect`      | `W_QUALITY_IMAGE_ASPECT`                                                                                                                                                    | warning                                                    | deterministic | An image drawn at an aspect the asset does not have                                                                                                     |
| `docx/contents-missing`  | `W_QUALITY_CONTENTS_MISSING`                                                                                                                                                | info; off unless a profile sets `minimumHeadings`          | deterministic | As many headings as the profile's `minimumHeadings`, or more, without a table of contents                                                               |
...
section's body size, across the section's usable width. Its own defaults are
book typography, 45–90 characters, for a caller who enables the rule directly;
the client- and technical-report profiles judge at 45–125 instead, by
measurement rather than preference: a body line on the bundled themes' own
pages runs 97 characters (minimal on A4) to 121 (devportal on Letter), the
reports of the #362 blind review shipped in that range, and 125 still catches
an 8pt body or half-inch margins. `executive-report` does not enable it. A section that lays
...
the document carries it: a base64 data URI, and in DOCX an inline SVG's viewBox. A
...
reaches the page — blocks, a statistic's figure, unit and description, a
list's items and the contents entries a theme styles included — against a
ceiling of eight that a profile or a policy can move through `maximumSizes`,
and it offers no fix, because which size to drop is a judgement. It counts per
document by decision rather than per page (#332): a report's pages share one
type system and no page paints a size the document does not, so the
document's ceiling bounds every page's; the static pass has no page model for
flowed text; and one dense page of the house blueprint already paints most of
the house sizes, so a per-page ceiling below the document's would fail the
blueprint's own pages. A deck, whose slide is its page, is counted both ways:
`pptx/size-count` holds each slide to `maximumSizesPerSlide` as well. A size
the theme's component defaults or the fit pass wrote into the props is counted
but never offered as a patch — the author wrote no member for a `replace` to
reach. The profiles' ceilings are measured, not chosen: a client report built
from the house blocks paints up to nine sizes (a `statistic` paints in the
theme's `stat` and `label` roles, so the kpi-row adds only its figure's scale
steps), a technical report eight against a ceiling of nine, a consulting deck nine
across the deck and six on its busiest slide; every checkpoint and verification document of that
archetype came in at or under its profile's number.
...
Findings map back to authored pointers through the document's text inventory: every painted string with its role, in reading order, read off the prepared document so a string a block compiled reports at the slot the author wrote. Matching folds both sides to lowercase alphanumerics (ligatures split, punctuation and bullets drop out) and searches one stream over the whole document, so a paragraph that breaks across a page is still one match; running heads and footers are matched first and their rows removed, so a page number never interleaves the two halves. Duplicate strings are attributed in reading order — "Total" in the second table finds the second "Total" — and an entry whose every occurrence was already claimed is `ambiguous` rather than guessed. A claim holds every word it matched, so a tracker that says "Revenue" cannot take the word from inside a title that opens with it. When the whole string is absent, its longest rendered prefix (at least 16 folded characters) maps it and reports the truncation.

A deck knows more than a report before it renders, and its entries say so. A slide is its own page, so slide text is looked for only on the page its slide exports to — hidden slides are left out of the export and out of the inventory — and its text box sits where the deck put it, so of the occurrences on that page only one that reaches into the box is its own, and the one lying most inside it wins. Text that overflowed onto other text shares its rows with the words it was drawn over and no run of the stream spells it; down its box's column, from the box's top, its own words still come in order with only those words between, and it is matched there, skipping the fewest. Text set at an angle comes out of the PDF in fragments no reading order can join: it claims an occurrence when one matches, is never reported missing, and is held to no box.
...
Reading order is decided from geometry, which is what lets a wrapped table cell match its own authored text. A row is parted into cells at gaps wider than half a line — and, where cell padding is tighter than that, at any gap the row's own word spacing cannot account for: within one row a word space holds to its median within a hundredth, since justification stretches every space of a line alike, while a cell boundary poppler ran together is close to twice it. A run of rows that resolves into more than one column reads column by column, so a cell's lines come out together even when no single row holds two cells — a figure centred between the two lines of the label beside it parts them exactly as a full row of cells does. Prose over the cells does not merge them into its own column: a row that sets two fragments inside one column shows the column was several all along, so what it holds so far is read first and each fragment opens a column of its own. A word joins a row only beside a word of about its own size (within twice its height), so a display title's word boxes, which span both lines of the small print set beside it, never read those lines one word from each at a time. Rotated text — a chart's value-axis title — reads as one run after the upright text of every page, so it can never fall between two lines of a paragraph: not beside a chart where poppler emitted it, and not at the top of a page a paragraph turns onto. Poppler's line grouping marks a rotated title's short words ("of") too. A bare-digits row inside a page band is a page number only when it stands alone, with no unclaimed row within a line of it, and its height recurs on another page; a table's numeric row that flowed into the band on one page is neither, and keeps its own occurrence.

Word boxes are font metrics, not ink. Poppler sizes a word by its face's ascent and descent — 1.1 to 1.6 times the type size across the bundled and stock faces — so a display title set tight runs its word boxes past a box it visibly fits, and a label set under a large figure sits inside the figure's descent. Spill and overlap are judged on a word's ink band, its box with a fifth of its height taken off the top and the bottom (`WORD_BOX_SLACK`): on the stock deck templates, font slack reaches 16% of a word's height past a box and 31% into the line under it, while text that truly overflows runs 70% of a line or more. A slide text box, whose position is known, holds its ink to its own top and bottom edges; a box that states only its size is held to that size.
...
A labelled mapping corpus (`preview-rendered-mapping-corpus.test.ts`, converter-dependent) pins the mapping on report- and deck-shaped documents: duplicate strings in two tables and a contents page, ligature-prone words, running chrome around a paragraph that breaks across pages, a family declared with a source that cannot load, a native chart's titles, a table broken badly across a page and a frame whose tail never rendered; and on slides, a takeaway beside a chart with a rotated axis title under a tracker that repeats the title's first word, figures that fold alike on two slides with a hidden slide between them, text past its box and past the slide's foot, and a text box placed off the slide — the fully clipped text a report cannot produce portably, since LibreOffice pulls an off-page frame back onto the page but exports a slide's off-canvas shape outside it. Every entry is labelled with the page it must land on; precision and recall against the labels are 1.0 on 59 entries, above the ≥0.95 / ≥0.90 targets of #344.

Measured on decks (2026-09-15). Every clip and spill the pass reported on the 20 decks of the #409/#422 verification sets that render — 91 findings — was the matcher's: re-mapped, each of those strings rendered whole, on its own slide, inside its box. They now report none, with no entry left ambiguous or missing. On the four stock deck templates, word boxes read at their font metrics had reported 100 findings; read as ink, 6 remain, each confirmed on the page (a second line in a box drawn for one, a label printed over a title). On the ground-truth harness's mutated decks the pass flags every spill of more than a line (31 of 31, including the 19 the static estimator leaves below `W_QUALITY_TEXT_OVERFLOW`) and none of the boxes that fit.

## Block coverage matrix

The rules are held to their own false-positive bar by a matrix over every JSON block the playground templates ship (#343). `buildMatrixInventory` in `jto-ops` reads each template with the extraction `jto://blocks` publishes, and states for every definition the conditions it is supported on — theme, canvas, font and slot edge — and a reason for each one it is not:

- A template on a bundled theme is authored against theme roles, so its definitions are supported on every bundled theme of the format, in the design faces and in the fallback faces (DejaVu Sans, which every LibreOffice bundles and which sets wider than the house faces). A report's definitions are supported on A4 and Letter. A deck's are supported on the slide shape the template is drawn on, because slot budgets are measured there: `consulting-deck-blocks` lays out on 4:3 inside the theme's safe area, but a 4:3 slide holds fewer words, so its budgets are no promise there.
- A template carrying its own theme object is authored against that object, with absolute geometry fitted to its own faces and slide, so its definitions are supported there only.
- A definition without slots has one document, so `min` and `max` are the same case.

The profile a template's definitions are judged under is the one its blueprint names (`client-report`, `technical-report`, `consulting-deck`), else the format default. A new template, definition or bundled theme gains its cases without an edit to the matrix.

`generateMatrixCases` turns the inventory into documents: each definition at the edges of its own slot schema — minimum and maximum cardinality, every string at its word budget in words of the length reports use, every figure at its widest, the worst-case chart — in a small report or deck around the template's own chrome, and each whole template with every invocation at that edge. The static half (`block-matrix.test.ts`) judges every supported case: it must validate, prepare, and come back with no warning-severity finding, and every finding, advice included, must point at something the author wrote. The rendered half (`preview-block-matrix.test.ts` in `mcp-server`) renders the riskiest subset through LibreOffice and holds it to the rendered pass's warnings and mapping; `JTO_BLOCK_MATRIX=full` renders every case, and `JTO_MATRIX_MANIFEST` names a file that receives the converter versions and, per case, every family requested, whether the document declared a source and whether the PDF embeds it. The converter-dependent CI job keeps that manifest as an artifact.

Clean cases are half the evidence. Each suite also injects defects and expects them back where they were caused: a slot one word or one item past its budget (`block_slot_budget` at the slot), a definition that composes from a block the document lacks or invokes itself on every expansion (`block_unknown_reference`, `block_expansion_limit` at the `ref` inside the definition), a framed paragraph that cannot fit its frame, and a deck box cut to a sliver with its fit taken away (a rendered spill at the invocation of the block that drew the box, quoting the text). `block-matrix-composition.test.ts` covers what no template ships: user-defined blocks that nest others, the same block invoked again, and documents mixing blocks, registered code plugins and primitives, where a finding in a plugin's output reports at the invocation that ran it.

Measured when the matrix gained decks (2026-09-16). Statically, 927 cases, all clean once three defects were fixed: the consulting deck frames were a percentage of the slide — the half-inch safe area on 16:9 but 27pt on 4:3 — and now span the theme's grid across; action titles held 17 words at the house sizes, not the 24 their budget allowed; the statement assertion held 11, not 14. Rendered, the report blocks were clean once the matcher learned to read a memo header's tab-aligned lines as the rows they are rather than as a table. The deck's widest cases were not: the cover title wraps a line more than its fit estimate allows and prints over the subtitle, and the statement's support and the action chart's takeaway run past their boxes — on the house theme a few points, on themes without type roles and in the fallback faces more. The spills are not monotonic in the word count, because a fit steps down a size only when its estimate overflows, so no budget fixes them alone: the deck fit estimate needs calibrating against rendered ground truth. Until then the rendered suite tolerates exactly those findings at those blocks and slots on the deck's widest cases (`DECK_FIT_DEFECTS`) and lists them in the manifest. The gallery templates themselves stay covered by `gallery-quality.test.ts`: none may carry a warning beyond the true findings and illustrative placeholders recorded for it.
...
| `executive-presentation` | PPTX   | 14pt minimum font; at most 70 body words per slide; at most five bullets of at most twelve words in a box; every content slide titled                                                                                                                                                                                                                                                                                                                                                                                      |
| `consulting-deck`        | PPTX   | Requires a slide footer with page numbers, `takeaway` and `source` slots, bounds action titles at two lines, at most 90 body words, at most five bullets of at most twelve words in a box, content inside the theme's safe area, every content slide titled, every image with alt text, every size on the theme scale with at most nine in the deck and six on a slide, and every title of one kind on one line                                                                                                            |
| `general`                | DOCX   | Default DOCX profile: integrity and information-design rules at their defaults, nothing required by structure                                                                                                                                                                                                                                                                                                                                                                                                              |
| `client-report`          | DOCX   | Requires a running head with page numbers on every section after the first, `takeaway` and `source` slots wherever a block declares them, promotes heading skips, rendered empty pages and under-filled pages to warnings, enables the three theme-consistency rules with at most nine sizes, judges the body measure at 45–125 characters, reports empty and untitled sections, headings a page break can strand and unlabelled figures, and requires at least one chart or table; the `client-report` blueprint names it |
| `technical-report`       | DOCX   | As `client-report`, but owes a figure only its `source`, never a `takeaway`, allows nine text sizes, and asks for a contents page from eight headings; the `technical-report` blueprint names it                                                                                                                                                                                                                                                                                                                           |
| `executive-report`       | DOCX   | Promotes heading skips from info to warning; reports empty and untitled sections and headings a page break can strand                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- replace a skipped heading level with the next valid level (not on a heading a block compiled);
- bind a heading to the paragraph under it with `keepNext`;
- snap an authored size to the nearest size on the theme's scale, or back to the theme's size for its role — in every place the size was written at once;
