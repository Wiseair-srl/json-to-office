...
| `rendered/page-underfilled` | `W_QUALITY_RENDERED_PAGE_UNDERFILLED` | info           | A docx page, other than the first and last, whose ink stops less than halfway down the body area (`context.fill` is the share used); needs pdftoppm for the ink profile |
...
| `client-report`          | DOCX   | Requires a running head with page numbers on every section after the first, `takeaway` and `source` slots wherever a block declares them, promotes heading skips, rendered empty pages and under-filled pages to warnings, and enables the three theme-consistency rules; the `client-report` blueprint names it |
