...
| `client-report`          | DOCX   | Requires a running head with page numbers on every section after the first, `takeaway` and `source` slots wherever a block declares them, promotes heading skips and rendered empty pages to warnings, and enables the three theme-consistency rules; the `client-report` blueprint names it |
