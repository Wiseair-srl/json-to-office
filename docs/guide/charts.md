...
All five built-in **PPTX** themes (`consulting`, `dark`, `minimal`, `vermilion`, `devportal`) define `accent4`–`accent6`, so a pptx chart gets six distinct colors out of the box; `consulting` additionally orders its series through `palette.chart`.
