...
Fourteen tools and `jto://` resources: ten for discovery, plus a document and a thumbnail for each bundled template. The [package README](https://github.com/Wiseair-srl/json-to-office/tree/main/packages/mcp-server#tools) documents every input and output field; the shape of the loop is:
