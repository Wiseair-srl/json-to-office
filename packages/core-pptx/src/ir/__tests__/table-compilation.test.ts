...
function compileTable(props: TableProps): PptxIrTableElement {
  const document = {
    name: 'pptx',
    props: { theme: THEME },
    children: [
  ...
  const { ir } = compileDocumentToIr(document);
...
function addTable(props: TableProps): EmittedTable {
  const element = compileTable(props);
...
describe('table-level fontWeight', () => {
  it('aliases the default face to the sub-family for a non-RIBBI weight', () => {
    const element = compileTable({ fontFace: 'Inter', fontWeight: 300 });
  ...
  it('falls back to the theme body font when no face is given', () => {
    const element = compileTable({ fontWeight: 500 });

    expect(element.defaults.fontFamily).toBe('Inter Medium');
    expect(addTable({ fontWeight: 500 }).opts.fontFace).toBe('Inter Medium');
  });

  it('keeps the canonical family and sets bold at 700', () => {
    const element = compileTable({ fontFace: 'Inter', fontWeight: 700 });
  ...
  it('leaves the face untouched when no weight is given', () => {
    const element = compileTable({ fontFace: 'Inter' });
  ...
  it('aliases a cell weight off the un-synthesized table family', () => {
    ...
    const { rows, opts } = addTable(props);
...
describe('a cell can opt out of an inherited table weight', () => {
  it('writes bold: false through so the table-level bold does not cascade', () => {
    ...
    expect(compileTable(props).rows[0].cells[0].formatting?.bold).toBe(false);
    expect(addTable(props).rows[0][0].options?.bold).toBe(false);
  });
  ...
  it('does not alias a bold:false cell onto a bold sub-family', () => {
    ...
    expect(
      compileTable(props).rows[0].cells[0].formatting?.fontFamily
    ).not.toBe('Inter Bold');
    expect(addTable(props).rows[0][0].options?.fontFace).not.toBe('Inter Bold');
  });
});
...
describe('cell formatting cascade', () => {
  it('puts table-level formatting in defaults, not in every cell', () => {
    const props = {
      fontSize: 14,
      fontFace: 'Georgia',
      color: 'primary',
    ...
    const { opts, rows } = addTable(props);
  ...
  it('defaults the size to the theme and the vertical alignment to middle', () => {
    const element = compileTable({});
  ...
  it('records only the properties a cell overrides', () => {
    const props = {
      rows: [
        [
          {
    ...
    const cell = compileTable(props).rows[0].cells[0];
    ...
    expect(addTable(props).rows[0][0].options).toEqual({
      color: 'FFFFFF',
      fontSize: 10,
      fontFace: 'Georgia',
      italic: true,
  ...
  it('resolves a theme colour token on the table and on a cell', () => {
    const element = compileTable({
      color: 'text',
...
describe('emoji variation selector', () => {
  it('forces text presentation on emoji-prone characters in a string cell', () => {
    const element = compileTable({ rows: [['✓ done', '★ starred']] });

    expect(element.rows[0].cells[0].text).toBe('✓\uFE0E done');
    expect(element.rows[0].cells[1].text).toBe('★\uFE0E starred');
  });

  it('forces text presentation in an object cell too', () => {
    const element = compileTable({
  ...
  it('suffixes every occurrence and leaves ordinary text alone', () => {
    const element = compileTable({ rows: [['✓ a ✓ b', 'plain text']] });

    expect(element.rows[0].cells[0].text).toBe('✓\uFE0E a ✓\uFE0E b');
    expect(element.rows[0].cells[1].text).toBe('plain text');
  });

  it('carries the suffixed text through to the emitted cell', () => {
    const { rows } = addTable({ rows: [['✓ done']] });
...
describe('borderRadius background shapes', () => {
  ...
  it('records the radius in inches on the IR and nothing about the technique', () => {
    const element = compileTable(roundedProps);
  ...
  it('draws a rounded header block and a rounded body block behind the table', () => {
    const { shapes } = addTable(roundedProps);
  ...
  it('leaves the corner cells transparent and fills every other cell', () => {
    const { rows } = addTable(roundedProps);
  ...
  it('suppresses the table outline and sizes the table to the column sum', () => {
    const { opts } = addTable(roundedProps);
  ...
  it('drops the outer edges and the header seam from the per-cell borders', () => {
    const { rows } = addTable({
      ...roundedProps,
  ...
  it('keeps the table-level border when no radius is asked for', () => {
    const element = compileTable({
    ...
    expect(
      addTable({ border: { type: 'dash', pt: 2, color: 'primary' } }).opts
        .border
