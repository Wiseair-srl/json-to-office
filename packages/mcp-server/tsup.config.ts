import { defineConfig } from 'tsup';
import { readFileSync } from 'fs';
import { join } from 'path';

const packageJson = JSON.parse(
  readFileSync(join(process.cwd(), 'package.json'), 'utf-8')
);

// Subpath entries are listed alongside their package: tsup's automatic
// dependency externalization matches bare specifiers, and `.../stdio` is not
// one of them — bundling the SDK would give the connection a second copy of
// its module-level state.
const commonExternal = [
  '@json-to-office/shared',
  '@json-to-office/shared-docx',
  '@json-to-office/shared-pptx',
  '@json-to-office/jto-ops',
  '@modelcontextprotocol/server',
  '@modelcontextprotocol/server/stdio',
  '@modelcontextprotocol/server/validators/ajv',
  '@sinclair/typebox',
  'ajv',
  'ajv-formats',
];

export default defineConfig([
  {
    entry: { cli: 'src/cli.ts' },
    format: ['esm'],
    dts: { compilerOptions: { incremental: false, composite: false } },
    splitting: false,
    sourcemap: true,
    clean: true,
    shims: true,
    minify: false,
    define: {
      __PACKAGE_VERSION__: JSON.stringify(packageJson.version),
    },
    external: commonExternal,
    esbuildOptions(options) {
      options.platform = 'node';
      options.target = 'node20';
    },
    banner: {
      js: '#!/usr/bin/env node',
    },
  },
  {
    entry: { index: 'src/index.ts' },
    format: ['esm'],
    dts: { compilerOptions: { incremental: false, composite: false } },
    splitting: false,
    sourcemap: true,
    clean: false,
    shims: true,
    minify: false,
    define: {
      __PACKAGE_VERSION__: JSON.stringify(packageJson.version),
    },
    external: commonExternal,
    esbuildOptions(options) {
      options.platform = 'node';
      options.target = 'node20';
    },
  },
]);
