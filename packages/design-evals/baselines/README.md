...
| `shipping-definition.json`                                    | —        | —                                                    | —               | the frozen definition, `clean-pages`, verified 2026-09-13 (pooled; per format it is not)        | —                               |
...
## The checkpoint set with the exhibit rule

`2026-09-08-checkpoint-after-exhibits-cold-server-4.4.4.json` repeats the set on the PR #405 build
(`96ed81c`): `docx/exhibit-required` on under `client-report` (one chart or
table of two or more columns), the narrative blueprint variant scaffolding a
data table, and `rendered/page-underfilled` judging the last page at a quarter.
Same eight briefs, three passes, cold, claude-sonnet-5, judged by
claude-opus-5, export server verified healthy before and every three minutes
during the run. 24/24 delivered, 0 failed, 0 export errors, median 2.5
iterations. Every delivered document carries a data table and 16 of 24 a
chart; the previous set had 13 charts and few tables.

Judged in one sitting against the before sheets
(`2026-09-08-rejudge-checkpoint-before-4.json` and `…-after-exhibits.json`):

| today's judge               | before | after-exhibits |
| --------------------------- | ------ | -------------- |
| would ship                  | 0/24   | 8/24           |
| level ≥4                    | 0/24   | 15/24          |
| median level                | 3      | 4              |
| level sum over 24           | 68     | 85             |
| briefs with a shippable run | 0/8    | 5/8            |

The before set's four sittings sum to 76, 73, 70 and 68: the judge keeps
drifting harsher, and this delta, +17, is measured against its harshest
sitting. In its own session the set scored 12 excellent and 6 ships, itself
the best of any set. The "argument in prose" complaint falls from 15 to 9
rationales; what the rationales name now are the empty cover (9) and the
stub last page (8).

The stub last page did not move: under the last-page rule both this set and
the flowing set carry 4 (`2026-09-08-page-fill-measure.json`,
`checkpoint-after-flow-last-page-rule` measures the earlier set under the same
rule). The agent saw the finding in three of the four and patched, but a
notes list that has spilled onto a new page has no move that pulls it back
short of shortening the page before it. A keep-with-previous option on the
...
## The shipping definition, calibrated (#409)

`2026-09-13-shipping-calibration.json` scores the six candidate definitions
...
| candidate         | reads                                               | n   | kappa [95%, by brief] | agree |
| ----------------- | --------------------------------------------------- | --- | --------------------- | ----- |
| `judge-v1`        | the v1 answer                                       | 64  | 0.38 [0.20, 0.55]     | 69%   |
| `judge-v2`        | the v2 answer                                       | 61  | 0.54 [0.31, 0.76]     | 77%   |
| `excellent`       | level ≥ 4                                           | 64  | 0.56 [0.24, 0.81]     | 78%   |
| `excellent-clean` | level ≥ 4, no integrity defect                      | 64  | 0.50 [0.13, 0.78]     | 75%   |
| `clean-pages`     | level ≥ 3, no integrity defect, ≤ 1 page defect     | 64  | 0.66 [0.40, 0.91]     | 83%   |
| `judge-v2-clean`  | the v2 answer, no integrity defect, ≤ 1 page defect | 63  | 0.68 [0.46, 0.88]     | 84%   |

The stated rule — highest kappa; within 0.05 of it, prefer no page term, then
fewer terms, then the listed order — keeps `judge-v2-clean` and `clean-pages`,
which tie on both preferences, so the listed order chooses **`clean-pages`**.
`shipping-definition.json` freezes it (hash `4477208543b3…`,
2026-09-13T20:11Z). It reads the question the product already asks, so
`jto_critique` does not change. Until `shipping-verification.json` records a
pass, a ship rate under it is advisory.

What the numbers carry:

- **The calibration kappa is optimistic.** It is measured on the labels that
...
- **Reviewer**: Sendability Review III, published after the freeze, asks #409's
  question (`2026-09-13-human-shipping-verification.json`, read out at
  21:56Z). The page stamps each answer's last save: all 30 were last saved
  between 21:53 and 21:55 UTC, so the stamps do not show how long the reading
  took.
...
**Result** (`shipping-verification.json`, attempt 1): `clean-pages` **passes
the stated target** — kappa 0.67 [0.40, 0.93] over 30 briefs, 83% agreement;
both ship 11, both hold 14, reviewer only 5, definition only 0. **Within a
format it does not track the reviewer:**

|                            | n   | reviewer ships | definition ships | both ship | both hold | reviewer only | kappa |
| -------------------------- | --- | -------------- | ---------------- | --------- | --------- | ------------- | ----- |
| client + technical reports | 16  | 15             | 11               | 11        | 1         | 4             | 0.26  |
| decks                      | 14  | 1              | 0                | 0         | 13        | 1             | 0.00  |

The pooled kappa is mostly the format split: the reviewer sends the reports and
holds the decks, and so does the definition. The integrity term makes both
halves of it. On reports it held all four the reviewer would send — rendered
clip or missing-text findings, as in calibration, where 3 of its 4 were
shipped — one of them also below the level floor; the level floor and the page
ceiling agree on the one report both hold. On decks it holds all fourteen,
each carrying rendered clip or spill findings; the reviewer held thirteen,
twelve with only "other" as the reason, so whether for the same defect is not
recorded.

By the rule written before any verdict the definition is verified, and the
record keeps the pass. What the pass supports is narrower than the number: a
ship rate under `clean-pages` separates a mixed corpus and says little inside
one format, so a reports-only or decks-only comparison should not rest on it.
The next calibration should score candidates per format and give the reviewer
a reason for clipped or overflowing content.
...
forty cold and assisted pairs: one pair at a time, sides randomised (cold on
the left in 20), "Which would you rather send?". Cold
`cd-pricing-strategy-workshop` is the clean rerun that replaced the
contaminated first run.

|                        | cold | assisted | tie |
| ---------------------- | ---- | -------- | --- |
| client reports (13)    | 8    | 4        | 1   |
| technical reports (11) | 7    | 3        | 1   |
| decks (16)             | 1    | 12       | 3   |
| all forty              | 16   | 19       | 5   |
